...
createNumberDivs = function(parentDiv)
{
    ...
    for(i=0; i<4; i++)
    {
        ...
        if(i==3){
			...
			zerobutton.setAttribute('class','borderClass buttonClass noPadding col-sm-offset-1 col-xs-offset-1 col-lg-offset-1 col-md-offset-1 col-xs-2 col-sm-2 col-md-2 col-lg-2 nopadding'); 
			document.getElementById('zeroButton'+(i+1)).innerHTML = buttonCnt;   
			colanbutton = document.createElement('button');
			document.getElementById('row'+(i+1)).appendChild(colanbutton);
			colanbutton.setAttribute('id','colanbutton'+(i+1));   
			colanbutton.setAttribute('onclick','appendNumbertoText(this.id)');
			colanbutton.setAttribute('class','borderClass buttonClass noPadding col-sm-offset-1 col-xs-offset-1 col-lg-offset-1 col-md-offset-1 col-xs-2 col-sm-2 col-md-2 col-lg-2 nopadding'); 
			document.getElementById('colanbutton'+(i+1)).innerHTML = ":";  
        }
        else{
			for(k=0; k<3; k++)
			{		 
