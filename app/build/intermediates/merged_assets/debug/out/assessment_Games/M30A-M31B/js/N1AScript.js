...
N1A.createDayDivs = function () {
	var parentDiv = document.getElementById("responseDays");
	for (i = 0; i < 7; i++) {
		var dayDiv = document.createElement("DIV");
		dayDiv.setAttribute("id", "day" + i);
		dayDiv.setAttribute("class", "col-lg-2 col-xm-2 col-sm-2 col-md-2  days borderClass codeCls buttonClass");
		dayDiv.setAttribute("onclick", "clickOnCode(this)");
		dayDiv.innerHTML = daysArray[N1A.langCtr][i];
		parentDiv.appendChild(dayDiv);
	}
	if (N1A.langCtr == 2)
		$(".days").css("font-size", "1.1vw");
...
ClickAns = function (ele) {
	N1A.attemptFlag = true;
	$("#AnswerDiv").css("pointer-events", "none");
	$(".responses").removeClass("clicked");
	$("#" + ele).addClass("clicked");
	scoredmarks = document.getElementById(ele).innerHTML;
}
...
function loadCalendarYears() {
	...
	for (var i = startYear; i <= endYear; i++) {
		...
		doc.onclick = (function () {
			...
			return function () {
				year = selectedYear;
				document.getElementById("curYear").innerHTML = 2019;
				loadCalendarDays();
				return year;
			}
		})();
