...
public final class VillageDao_Impl implements VillageDao {
  ...
  @Override
  public String getVillageName(String villName, String stateName, String districtName) {
    final String _sql = "SELECT villageName from Village WHERE villageName=? AND villageState=? AND villageDistrict=?";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 3);
    int _argIndex = 1;
    if (villName == null) {
      _statement.bindNull(_argIndex);
    } else {
      _statement.bindString(_argIndex, villName);
    }
    _argIndex = 2;
    if (stateName == null) {
      _statement.bindNull(_argIndex);
    } else {
      _statement.bindString(_argIndex, stateName);
    }
    _argIndex = 3;
    if (districtName == null) {
      _statement.bindNull(_argIndex);
    } else {
      _statement.bindString(_argIndex, districtName);
    }
    final Cursor _cursor = __db.query(_statement);
    try {
